...
function main() {
	...
	const
        ...
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-lower';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_MAIN = '^https://epl.taxcom.ru/$', // тут жму на кнопку "Подписать"
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-lower$', // запуск плагина
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>Плагин: all-titles-lower. Кликает нижний ЭПЛ из списка на странице /at_signing. Перехвачено алертов: '+ num +'</div>');
