...
function main() {
	...
	const
        PLUGIN_VERSION = 'all-titles-lower-fast',
        
        TITLE_T5 = 'т5 - показания одометра при заезде',
        
        URL_AT_SIGNING = 'https://epl.taxcom.ru/at_signing/',
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-lower-fast';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_MAIN = '^https://epl.taxcom.ru/$', // тут жму на кнопку "Подписать"
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-lower-fast$', // запуск плагина
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>Плагин: all-titles-lower-fast. Кликает нижний ЭПЛ из списка на странице /at_signing. Обрабатывает зависание кнопки "подписать"</div>');
