...
function main() {
	...
	const
        PLUGIN_VERSION = 'all-titles-penultimate-if-2-or-more',
        
        TITLE_T5 = 'т5 - показания одометра при заезде',
        
        URL_AT_SIGNING = 'https://epl.taxcom.ru/at_signing/',
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-penultimate-if-2-or-more';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_MAIN = '^https://epl.taxcom.ru/$', // тут жму на кнопку "Подписать"
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-penultimate-if-2-or-more$', // запуск плагина
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>&#128315;&#128315; ПРЕДПОСЛЕДНИЙ: перехвачено '+ num +'</div>');
    ...
    function actionAtSigning() {
        ...
        if(links.length < 2) {
            ...
            setTimeout(()=>{ links[links.length-2].click(); }, DELAY_LINK_CLICK_MS);
        }
    }
    ...
    function actionWaybill() {
        ...
        if(!btn) {
            ...
            setBorderStyle(BORDER_STYLE_OK, false);
            setTimeout(()=>{ btn.click(); }, DELAY_LINK_CLICK_MS);
        }
    }
    /**
    ...
    function handleAlert() {
        ...
        setTimeout(()=>{
            ...
            if(typeof window.sessionStorage.alerts_counter == 'undefined') {
                window.sessionStorage.alerts_counter = 0;
            }
            window.sessionStorage.alerts_counter++;
