...
function main() {
	...
	const
        PLUGIN_VERSION = 'all-titles-lower-t4-extended',
        
        TITLE_T4 = 'т4 - показания одометра при выезде',
        TITLE_T5 = 'т5 - показания одометра при заезде',
        
        URL_LIST_ALL = 'https://epl.taxcom.ru/all/',
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-lower-t4-extended';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-lower-t4-extended$', // запуск плагина
		URL_PATTERN_LIST_ALL = '^https://epl.taxcom.ru/all/$', // тут собираю урлы ЭПЛов
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>Плагин: '+ PLUGIN_VERSION +'. Кликает нижний ЭПЛ из списка на странице "ВСЕ ЭПЛ". Для Т4 - обрабатывает так же кнопки ОТКРЫТЬ -> ОТПРАВИТЬ.</div>');
