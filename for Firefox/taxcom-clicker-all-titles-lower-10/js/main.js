...
function main() {
    ...
    const
        ...
        DELAY_LINK_CLICK_MS = 10000, // задержка между мереходами по страницам
        DELAY_REDIRECT_TO_PREVENT_ALERTS = 60000, // при заглушеных алертах - перенаправлять на список
        DELAY_PAGE_RELOAD = 10000; // через сколько перезагружать страницу
    
	/* паттерны урлов и идентификаторы страниц */
	const
        PLUGIN_VERSION = 'all-titles-lower',
        
        TITLE_T5 = 'т5 - показания одометра при заезде',
        
        URL_AT_SIGNING = 'https://epl.taxcom.ru/at_signing/',
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-lower-10';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_MAIN = '^https://epl.taxcom.ru/$', // тут жму на кнопку "Подписать"
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-lower-10$', // запуск плагина
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>&#128315; НИЖНИЙ: перехвачено: '+ num +'</div>');
