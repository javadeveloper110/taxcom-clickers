...
class Clicker {
    ...
    URL_START = 'https://epl.taxcom.ru/at_signing/?start-title5-all-2';
    ...
    URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-title5-all-2'; // запуск плагина
    ...
    checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>Плагин: title5-all. Обрабатывает ЭПЛ на сайте epl.taxcom.ru. Обрабатывает все ЭПЛ на странице /at_signing, с пагинацией, запоминает номер страницы. Перехвачено алертов: '+ num +'</div>');
    ...
    getWaybillTitle5ButtonSign() {
        ...
        for(let i = 0; i < buttons.length; i++) {
            let regex = /подписать/;
            //console.info(i, buttons[i].disabled, regex.test(buttons[i].firstChild.textContent.toLowerCase()));
    ...
    actionWaybillTitle5() {
        ...
        if(btn) {
            ...
            this.setBorderStyle(self.BORDER_STYLE_OK, false);
            setTimeout(()=>{ btn.click(); }, 1000);
        } else {
            console.info("кнопка не найдена");
            setTimeout(()=>{ location.href = this.getLastPageUrl(); }, this.DELAY_PAGE_RELOAD);
        }
    }
     ...
     */
    forgetLastPageUrl() {
        console.info("забываю последнюю страницу");
     ...
     */
    rememberLastPageUrl() {
        console.info("запоминаю последнюю страницу:", document.location.href);
    ...
    actionAtSigning() {
        ...
        if(link) {
            ...
            setTimeout((self, link)=>{
                // перехожу
                link.click();
            }, this.DELAY_LINK_CLICK_MS, this, link);
        } else if(next_button) {
            this.setBorderStyle(this.BORDER_STYLE_OK, false);
    ...
    actionWaybill() {
        ...
        console.info("ищу кнопку 'заполнить'");
