...
function main() {
	...
	const
        PLUGIN_VERSION = 'all-titles-upper-improved',
        
        TITLE_T5 = 'т5 - показания одометра при заезде',
        
        URL_AT_SIGNING = 'https://epl.taxcom.ru/at_signing/',
        URL_START = 'https://epl.taxcom.ru/at_signing/?start-all-titles-upper-improved';
        
        MILEAGE_PATTERN = '^показания одометра: [0-9]+$',
        
		URL_PATTERN_MAIN = '^https://epl.taxcom.ru/$', // тут жму на кнопку "Подписать"
		URL_PATTERN_START_PLUGIN = '^https://epl.taxcom.ru/at_signing/\\?start-all-titles-upper-improved$', // запуск плагина
    ...
    function checkPluginVersion() {
        ...
        if(res) {
            let
                ...
                fragment = create('<div>Плагин: all-titles-upper-improved. Кликает верхний ЭПЛ из списка /at_signing, если на странице больше двух записей. Обрабатывает зависание кнопки "подписать"</div>');
    ...
    window.alert = function() {
        ...
        if(typeof window.sessionStorage.alerts_counter_plugin_upper == 'undefined') {
            window.sessionStorage.alerts_counter_plugin_upper = 0;
        }
        window.sessionStorage.alerts_counter_plugin_upper++;
        setTimeout(()=>{ location.href = URL_AT_SIGNING; }, DELAY_PAGE_RELOAD);
    }
